...
import rospy
from sensor_msgs.msg import Image   # sensor_msg/Image 메시지 구독
from std_msgs.msg import Int32      # std_msg/Int32 메시지 구독
...
class lane_detection():
    ...
    def find_start_point(self, binary_img):
        ...
        THH, THL = int(area * 0.6), int(area * 0.06)
        ...
        # 우측의 노이즈만 심한 경우
        elif (THL <= left_noise or left_noise <= THH) & (right_noise > THH or right_noise < THL):
            ...
            cv2.line(unit_img, (right_start, 0), (right_start, h), (0,0,255), thickness=3) 
        # 좌측의 노이즈만 심한 경우
        elif (left_noise > THH or left_noise < THL) & (THL <= right_noise or right_noise <= THH):
            self.MODE = 'left_auto'
    ...
    def calcul_error(self, draw_img, ltx, rtx):
        ...
        if self.MODE == 'tracking':
            ...
            a = int(ltx[0]) - int(ltx[int(h//2)])
            b = int(ltx[int(h//2)]) - int(ltx[h-1])
            self.error = int(10 + (a+b)*0.8)
            cv2.line(draw_img, (lane_center,0), (lane_center,h), (0,255,255), thickness=3)

        elif self.MODE == 'left_auto':
            c = int(rtx[0]) - int(rtx[int(h//2)])
            d = int(rtx[int(h//2)]) - int(rtx[h-1])
            self.error = int(-5 + (c+d)*0.1)
            cv2.line(draw_img, (lane_center,0), (lane_center,h), (255,0,255), thickness=3)
    ...
    def process(self, img):
        ...
        #v2.imshow('Combined Image', combined_image)
        cv2.waitKey(1)
        ''' -------------------------------------------------------------- '''
